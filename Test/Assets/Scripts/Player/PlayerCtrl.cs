...
public class PlayerCtrl : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Return))
        {
            ...
            PauseCtrl.self.Pause(!PauseCtrl.self.isPaused);
            SetAnimSpeed(1 - anim.speed);
            body.velocity = Vector2.zero; //freeze velocity
    ...
    private void FixedUpdate()
    {
        ...
        if (!PauseCtrl.self.isPaused && !HurtBool)
        { //can act only if game is not paused and player is not hurt
            Run();
            Jump();
            SetAnimator();
        }
    ...
    void Jump()
    {
        ...
        { //when falling after high jump
            //gravity is multiplied
            body.velocity += Vector2.up * Physics2D.gravity.y * (HighFallMultiplier - 1) * Time.deltaTime;
        }
        else if (JumpingUpBool && !(Input.GetKey(KeyCode.Space)))
        { //low jump: is jumping up and not holding jump key
